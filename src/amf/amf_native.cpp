/**
 * @file src/amf/amf_native.cpp
 * @brief Implementation of standalone AMF encoder with platform-native texture input.
 */
...
namespace amf {
  ...
  void AMF_STD_CALL
    amf_native::input_surface_release_observer_t::OnSurfaceDataRelease(::amf::AMFSurface *surface) {
    if (owner) {
      owner->on_platform_surface_released(slot_index, surface);
      owner->on_input_surface_released(slot_index);
    }
  }

  void
    amf_native::on_input_surface_released(std::size_t slot_index) noexcept {
  ...
  bool
    amf_native::ensure_input_surface_count(std::size_t count) {
    count = std::min(count, input_surface_ring.size());
    for (std::size_t index = 0; index < count; ++index) {
      if (!allocate_platform_surface(index)) {
        BOOST_LOG(error) << "AMF: failed to allocate native input surface " << index;
        return false;
      }
    }
    return true;
  ...
  AMF_SURFACE_FORMAT
  amf_native::get_amf_format(platf::pix_fmt_e buffer_format, int bit_depth) {
  ...
  bool
    amf_native::configure_encoder(const amf_config &config, const video::config_t &client_config, const video::sunshine_colorspace_t &colorspace) {
  ...
  bool
    amf_native::create_encoder(const amf_config &config, const video::config_t &client_config, const video::sunshine_colorspace_t &colorspace, platf::pix_fmt_e buffer_format) {
    // Determine video format from client config
    video_format = client_config.videoFormat;
    current_config = client_config;

    // Resolve the platform AMF runtime only when native encoding is probed.
    if (!factory && !load_amf_runtime(factory)) {
      return false;
    }
    ...
    // Attach the platform graphics device. Windows uses D3D11 while Linux uses
    // the same Vulkan device that imports and converts captured DMA-BUF frames.
    res = initialize_platform_context(context);
    if (res != AMF_OK) {
      BOOST_LOG(error) << "AMF: platform context initialization failed, error: " << res;
      return false;
    }
    ...
    // Create the rotating platform surfaces that are both conversion targets and
    // native AMF inputs. Platform adapters keep this path GPU-only.
    clear_platform_surfaces();
    if (!configure_platform_surfaces(
          buffer_format,
          colorspace.bit_depth,
          client_config.width,
          client_config.height
        )) {
      BOOST_LOG(error) << "AMF: platform does not support the requested native surface format";
      return false;
    }
  ...
  void
    ...
    // Release app-owned native surfaces while the platform context (and its
    // Vulkan/D3D synchronization facilities) is still valid.
    clear_platform_surfaces();
    if (context) {
      context->Terminate();
      context = nullptr;
  ...
  amf_encoded_frame
    amf_native::extract_encoded_frame(const ::amf::AMFDataPtr &output_data) {
  ...
  void
    amf_native::output_pump(std::stop_token stop_token) noexcept {
    configure_output_thread();
    ...
    try {
      while (!stop_token.stop_requested()) {
...

        {
          ...
          {
            std::lock_guard lock(state_mutex);
            if (platform_device_failed("QueryOutput")) {
              output_fatal = true;
              fatal = true;
  ...
  amf_encode_result
    amf_native::encode_frame(uint64_t frame_index, bool force_idr) {
    ...
    if (duplicate_source_slot) {
      if (!copy_platform_surface(slot_index, *duplicate_source_slot)) {
        BOOST_LOG(error) << "AMF: failed to duplicate a repeated native input surface";
        result.fatal = true;
        return result;
      }
    }
    if (platform_device_failed("native input submission")) {
      result.fatal = true;
      return result;
    }
    ...
    auto res = create_platform_amf_surface(
      context,
      slot_index,
      &surface,
    ...
    if (res != AMF_OK || !surface) {
      BOOST_LOG(error) << "AMF: native surface wrapping failed, error: " << res;
      if (platform_device_failed("native surface wrapping")) {
        result.fatal = true;
    ...
    if (res != AMF_OK) {
      ...
      if (platform_device_failed("SubmitInput")) {
        result.fatal = true;
        return result;
      }
  ...
  amf_encode_result
    amf_native::drain_output(std::chrono::milliseconds timeout) {
    amf_encode_result result;
    std::unique_lock lock(state_mutex);
  ...
  bool
    amf_native::invalidate_ref_frames(uint64_t first_frame, uint64_t last_frame) {
    std::lock_guard lock(state_mutex);
  ...
  bool
    amf_native::set_bitrate(int bitrate_kbps) {
  ...
  bool
    amf_native::set_hdr_metadata(const std::optional<amf_hdr_metadata> &metadata) {
  ...
  void *
    amf_native::get_input_texture() {
    return platform_surface(0);
  }

  void *
    amf_native::acquire_input_surface_for_render() {
    std::unique_lock lock(state_mutex);
    if (prepared_input_surface_slot) {
      return platform_surface(*prepared_input_surface_slot);
    }
    ...
    return platform_surface(*slot);
  }

  void
    amf_native::cancel_input_surface_for_render() {
    std::lock_guard lock(state_mutex);
    if (!prepared_input_surface_slot) {
      return;
    }
